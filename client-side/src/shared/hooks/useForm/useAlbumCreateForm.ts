...
import { useCreateAlbum } from "../ApiHooks/useAlbums/useAlbums";

export const useAlbumCreateForm = (onClose: () => void) => {
  const { createAlbum, isLoadingCreate } = useCreateAlbum();
  ...
  return { isLoading:isLoadingCreate, form, onSubmit, imageUpload };
};
